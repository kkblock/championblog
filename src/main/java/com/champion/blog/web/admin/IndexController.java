...
import com.champion.blog.utils.GsonUtils;
...
public class IndexController extends BaseController {
    ...
    public RestResponseBo upPwd(@RequestParam String oldPassword, @RequestParam String password, HttpServletRequest request, HttpSession session) {
        ...
        if (!users.getPassword().equals(ChampionUtils.MD5encode(users.getUsername() + oldPassword))) {
        ...
        try {
            UserVo temp = new UserVo();
            temp.setUid(users.getUid());
            String pwd = ChampionUtils.MD5encode(users.getUsername() + password);
